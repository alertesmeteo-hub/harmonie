(function () {
    ...
    function initApp(app) {
        ...
        function showTableMessage(message, error) {
            ...
            putMessage(stormBody, message, error, 18);
            putMessage(snowBody, message, error, 14);
            if (stormSummary) {
            ...
            return fetchJson(baseUrl + '/index.json', { cache: 'no-cache' })
                .then(function (payload) {
                    ...
                    meta.textContent = 'Run du ' + run + ' • résolution ' + resolution +
                        ((payload.model && payload.model.storm_diagnostics) ? ' • orages + neige P3' : '');
        ...
        function renderForecast(departmentData, commune) {
            ...
            var hasSnowColumns = Object.prototype.hasOwnProperty.call(columnIndex, 'snow_risk_code')
                && Object.prototype.hasOwnProperty.call(columnIndex, 'thickness_1000_500_dam')
                && Object.prototype.hasOwnProperty.call(columnIndex, 'geopotential_925_m');
            var chartLabels = [];
            var chartHourLabels = [];
            var chartDayKeys = [];
            var chartDayTokens = [];
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                capeCell.title = 'Diagnostic CAPE approché sur le profil P3 ; les valeurs quasi nulles ne sont pas affichées.';
                stormRow.appendChild(capeCell);
                var cin = value(values, 'cin_jkg');
                var cinCell = createCell('td', 'CIN');
                cinCell.textContent = finite(cin) && Math.abs(Number(cin)) >= 5 ? formatNumber(cin, 0) + ' J/kg' : '—';
                cinCell.title = 'Diagnostic CIN approché sur le profil P3 ; les valeurs quasi nulles ne sont pas affichées.';
                stormRow.appendChild(cinCell);
                appendNumber(stormRow, 'LCL', value(values, 'lcl_m'), 0, ' m');
                appendNumber(stormRow, 'K Index', value(values, 'k_index'), 1, '');
                appendNumber(stormRow, 'Total Totals', value(values, 'total_totals'), 1, '');
                ...
                appendNumber(stormRow, 'T 500 hPa', value(values, 'temperature_500_c'), 0, ' °C');
                appendNumber(stormRow, 'RH 700 hPa', value(values, 'humidity_700_pct'), 0, ' %');
                ...
                [
                    ['Cisaillement 0–1 km', value(values, 'shear_0_1_ms'), 1, ' m/s'],
                    ['Cisaillement 0–3 km', value(values, 'shear_0_3_ms'), 1, ' m/s'],
                    ['Cisaillement 0–6 km', value(values, 'shear_0_6_ms'), 1, ' m/s'],
                    ['SRH 0–1 km', value(values, 'srh_0_1_m2s2'), 0, ' m²/s²'],
                    ['SRH 0–3 km', value(values, 'srh_0_3_m2s2'), 0, ' m²/s²'],
                    ['Iso 0 °C', value(values, 'freezing_level_m'), 0, ' m'],
                    ['Iso −20 °C', value(values, 'minus20_level_m'), 0, ' m'],
                    ['ω 500 hPa', value(values, 'omega_500_pas'), 2, ' Pa/s']
                ...
                appendNumber(snowRow, 'Épaisseur 1000–500', value(values, 'thickness_1000_500_dam'), 0, ' dam', 'hkw-thickness');
                ...
                appendNumber(snowRow, 'Neige au sol', value(values, 'snow_depth_cm'), 1, ' cm');
                ...
                appendProfileSection('Températures (°C)', [
                    ['2 m', temp, 0, ' °C'],
                    ['z975', value(values, 'temperature_975_c'), 0, ' °C'],
                    ['z950', value(values, 'temperature_950_c'), 0, ' °C'],
                    ['z925', value(values, 'temperature_925_c'), 0, ' °C'],
                    ['z900', value(values, 'temperature_900_c'), 0, ' °C'],
                    ['z850', value(values, 'temperature_850_c'), 0, ' °C']
                ]);
                appendProfileSection('Géopotentiel (m)', [
                    ['z975', value(values, 'geopotential_975_m'), 0, ' m'],
                    ['z950', value(values, 'geopotential_950_m'), 0, ' m'],
                    ['z925', value(values, 'geopotential_925_m'), 0, ' m'],
                    ['z900', value(values, 'geopotential_900_m'), 0, ' m'],
                    ['z850', value(values, 'geopotential_850_m'), 0, ' m']
                ]);
            ...
            if (stormSummary) {
                ...
                if (!hasStormColumns) {
                    ...
                    stormSummary.textContent = 'Risque orageux maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxRisk.icon + ' ' + maxRisk.label + ' • diagnostics HARMONIE P3.';
                }
            }
            if (snowSummary) {
                ...
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • neige HGTY HARMONIE P3.';
                }
            }
            ...
            if (altitudeLine) {
                altitudeLine.textContent = finite(altitude)
                    ? 'Altitude de ' + cityName + ' : ≈ ' + formatNumber(altitude, 0) + ' m (point de grille HARMONIE)'
                    : 'Altitude de ' + cityName + ' : — (relancez le workflow GitHub avec les scripts v2.6)';
